...
router.post("/signup", (req, res, next) => {
  ...
  User.findOne({ email })
    .then((foundUser) => {
      ...
      User.create({ email, password: hashedPassword, name, profilePicURL })
        .then((createdUser) => {
          // Deconstruct the newly created user object to omit the password
          // We should never expose passwords publicly
          const { email, name, _id, profilePicURL } = createdUser;

          // Create a new object that doesn't expose the password
          const user = { email, name, _id, profilePicURL };

          const payload = { _id, email, name, profilePicURL };
